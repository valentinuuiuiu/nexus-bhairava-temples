"""
Shopify Addon for NewZyon/PAI-KAN Architecture
Temple 43 - Shopify Guardian
Purpose: E-Commerce
...
class ShopifyAddon:
    """
    Shopify integration addon for MCP server
    Temple 43 of the 64 Bhairava Guardian Temples
    Guardian Purpose: E-Commerce
    """

    API_BASE = "https://mystore.myshopify.com/admin/api"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("SHOPIFY_TOKEN")
        if not self.token and "SHOPIFY_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing SHOPIFY_TOKEN. "
                f"Get your token from the Shopify developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "SHOPIFY_TOKEN" == "NO_AUTH_REQUIRED":
