"""
Auth0 Addon for NewZyon/PAI-KAN Architecture
Temple 56 - Auth0 Guardian
...
class Auth0Addon:
    """
    Auth0 integration addon for MCP server
    Temple 56 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Authentication
    """

    API_BASE = "https://yourdomain.auth0.com/api/v2"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("AUTH0_TOKEN")
        if not self.token and "AUTH0_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing AUTH0_TOKEN. "
                f"Get your token from the Auth0 developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "AUTH0_TOKEN" == "NO_AUTH_REQUIRED":
