"""
Firebase Addon for NewZyon/PAI-KAN Architecture
Temple 30 - Firebase Guardian
Purpose: Realtime DB
...
class FirebaseAddon:
    """
    Firebase integration addon for MCP server
    Temple 30 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Realtime DB
    """

    API_BASE = "https://firestore.googleapis.com/v1"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("FIREBASE_TOKEN")
        if not self.token and "FIREBASE_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing FIREBASE_TOKEN. "
                f"Get your token from the Firebase developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "FIREBASE_TOKEN" == "NO_AUTH_REQUIRED":
