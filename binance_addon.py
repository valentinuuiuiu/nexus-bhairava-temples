"""
Binance Addon for NewZyon/PAI-KAN Architecture
Temple 45 - Binance Guardian
...
class BinanceAddon:
    """
    Binance integration addon for MCP server
    Temple 45 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Crypto Trading
    """

    API_BASE = "https://api.binance.com/api/v3"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("BINANCE_API_KEY")
        if not self.token and "BINANCE_API_KEY" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing BINANCE_API_KEY. "
                f"Get your token from the Binance developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "BINANCE_API_KEY" == "NO_AUTH_REQUIRED":
