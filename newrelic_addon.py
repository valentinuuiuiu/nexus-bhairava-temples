"""
New Relic Addon for NewZyon/PAI-KAN Architecture
Temple 59 - New Relic Guardian
Purpose: APM
...
class NewrelicAddon:
    """
    New Relic integration addon for MCP server
    Temple 59 of the 64 Bhairava Guardian Temples
    Guardian Purpose: APM
    """

    API_BASE = "https://api.newrelic.com/v2"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("NEWRELIC_API_KEY")
        if not self.token and "NEWRELIC_API_KEY" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing NEWRELIC_API_KEY. "
                f"Get your token from the New Relic developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "NEWRELIC_API_KEY" == "NO_AUTH_REQUIRED":
