"""
Prometheus Addon for NewZyon/PAI-KAN Architecture
Temple 58 - Prometheus Guardian
Purpose: Metrics
...
class PrometheusAddon:
    """
    Prometheus integration addon for MCP server
    Temple 58 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Metrics
    """

    API_BASE = "http://localhost:9090/api/v1"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("PROMETHEUS_URL")
        if not self.token and "PROMETHEUS_URL" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing PROMETHEUS_URL. "
                f"Get your token from the Prometheus developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "PROMETHEUS_URL" == "NO_AUTH_REQUIRED":
