"""
HubSpot Addon for NewZyon/PAI-KAN Architecture
Temple 50 - HubSpot Guardian
...
class HubspotAddon:
    """
    HubSpot integration addon for MCP server
    Temple 50 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Marketing CRM
    """
    ...
    def __init__(self, token: Optional[str] = None):
        ...
        if not self.token and "HUBSPOT_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing HUBSPOT_TOKEN. "
                f"Get your token from the HubSpot developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "HUBSPOT_TOKEN" == "NO_AUTH_REQUIRED":
