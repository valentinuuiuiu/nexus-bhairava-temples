"""
AWS Addon for NewZyon/PAI-KAN Architecture
Temple 11 - AWS Guardian
...
class AwsAddon:
    """
    AWS integration addon for MCP server
    Temple 11 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Cloud Infrastructure
    """

    API_BASE = "https://amazonaws.com"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("AWS_ACCESS_KEY")
        if not self.token and "AWS_ACCESS_KEY" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing AWS_ACCESS_KEY. "
                f"Get your token from the AWS developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "AWS_ACCESS_KEY" == "NO_AUTH_REQUIRED":
