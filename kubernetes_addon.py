"""
Kubernetes Addon for NewZyon/PAI-KAN Architecture
Temple 15 - Kubernetes Guardian
...
class KubernetesAddon:
    """
    Kubernetes integration addon for MCP server
    Temple 15 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Orchestration
    """

    API_BASE = "https://kubernetes.default.svc"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("KUBE_TOKEN")
        if not self.token and "KUBE_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing KUBE_TOKEN. "
                f"Get your token from the Kubernetes developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "KUBE_TOKEN" == "NO_AUTH_REQUIRED":
