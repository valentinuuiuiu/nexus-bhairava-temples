"""
WooCommerce Addon for NewZyon/PAI-KAN Architecture
Temple 44 - WooCommerce Guardian
...
class WoocommerceAddon:
    """
    WooCommerce integration addon for MCP server
    Temple 44 of the 64 Bhairava Guardian Temples
    Guardian Purpose: WordPress Commerce
    """

    API_BASE = "https://yourstore.com/wp-json/wc/v3"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("WC_CONSUMER_KEY")
        if not self.token and "WC_CONSUMER_KEY" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing WC_CONSUMER_KEY. "
                f"Get your token from the WooCommerce developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "WC_CONSUMER_KEY" == "NO_AUTH_REQUIRED":
