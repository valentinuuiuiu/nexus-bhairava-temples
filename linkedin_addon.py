"""
LinkedIn Addon for NewZyon/PAI-KAN Architecture
Temple 34 - LinkedIn Guardian
...
class LinkedinAddon:
    """
    LinkedIn integration addon for MCP server
    Temple 34 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Professional Network
    """

    API_BASE = "https://api.linkedin.com/v2"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("LINKEDIN_TOKEN")
        if not self.token and "LINKEDIN_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing LINKEDIN_TOKEN. "
                f"Get your token from the LinkedIn developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "LINKEDIN_TOKEN" == "NO_AUTH_REQUIRED":
