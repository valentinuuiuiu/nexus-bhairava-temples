"""
GCP Addon for NewZyon/PAI-KAN Architecture
Temple 12 - GCP Guardian
...
class GcpAddon:
    """
    GCP integration addon for MCP server
    Temple 12 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Google Cloud
    """

    API_BASE = "https://googleapis.com"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("GCP_TOKEN")
        if not self.token and "GCP_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing GCP_TOKEN. "
                f"Get your token from the GCP developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "GCP_TOKEN" == "NO_AUTH_REQUIRED":
