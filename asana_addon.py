"""
Asana Addon for NewZyon/PAI-KAN Architecture
Temple 10 - Asana Guardian
Purpose: Tasks
...
class AsanaAddon:
    """
    Asana integration addon for MCP server
    Temple 10 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Tasks
    """
    ...
    def __init__(self, token: Optional[str] = None):
        ...
        if not self.token and "ASANA_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing ASANA_TOKEN. "
                f"Get your token from the Asana developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "ASANA_TOKEN" == "NO_AUTH_REQUIRED":
