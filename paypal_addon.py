"""
PayPal Addon for NewZyon/PAI-KAN Architecture
Temple 42 - PayPal Guardian
Purpose: Payments
...
class PaypalAddon:
    """
    PayPal integration addon for MCP server
    ...
    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("PAYPAL_CLIENT_ID")
        if not self.token and "PAYPAL_CLIENT_ID" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing PAYPAL_CLIENT_ID. "
                f"Get your token from the PayPal developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "PAYPAL_CLIENT_ID" == "NO_AUTH_REQUIRED":
