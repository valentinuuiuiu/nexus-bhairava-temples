"""
Telegram Addon for NewZyon/PAI-KAN Architecture
Temple 32 - Telegram Guardian
Purpose: Messaging
...
class TelegramAddon:
    """
    Telegram integration addon for MCP server
    Temple 32 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Messaging
    """
    ...
    def __init__(self, token: Optional[str] = None):
        ...
        if not self.token and "TELEGRAM_BOT_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing TELEGRAM_BOT_TOKEN. "
                f"Get your token from the Telegram developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "TELEGRAM_BOT_TOKEN" == "NO_AUTH_REQUIRED":
