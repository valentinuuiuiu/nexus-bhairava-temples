"""
Cloudflare Addon for NewZyon/PAI-KAN Architecture
Temple 18 - Cloudflare Guardian
...
class CloudflareAddon:
    """
    Cloudflare integration addon for MCP server
    Temple 18 of the 64 Bhairava Guardian Temples
    Guardian Purpose: CDN & Security
    """

    API_BASE = "https://api.cloudflare.com/client/v4"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("CLOUDFLARE_TOKEN")
        if not self.token and "CLOUDFLARE_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing CLOUDFLARE_TOKEN. "
                f"Get your token from the Cloudflare developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "CLOUDFLARE_TOKEN" == "NO_AUTH_REQUIRED":
