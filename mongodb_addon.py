"""
MongoDB Addon for NewZyon/PAI-KAN Architecture
Temple 22 - MongoDB Guardian
Purpose: Document DB
...
class MongodbAddon:
    """
    MongoDB integration addon for MCP server
    Temple 22 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Document DB
    """

    API_BASE = "mongodb://localhost:27017"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("MONGO_URI")
        if not self.token and "MONGO_URI" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing MONGO_URI. "
                f"Get your token from the MongoDB developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "MONGO_URI" == "NO_AUTH_REQUIRED":
