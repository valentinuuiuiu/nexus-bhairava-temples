...
class NotionAddon:
    ...
    Guardian Purpose: Notes & Knowledge Base
    """
    ...
    def __init__(self, token: Optional[str] = None):
        ...
        if not self.token and "NOTION_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing NOTION_TOKEN. "
                f"Get your token from the Notion developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "NOTION_TOKEN" == "NO_AUTH_REQUIRED":
