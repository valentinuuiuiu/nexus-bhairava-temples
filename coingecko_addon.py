"""
CoinGecko Addon for NewZyon/PAI-KAN Architecture
Temple 46 - CoinGecko Guardian
...
class CoingeckoAddon:
    """
    CoinGecko integration addon for MCP server
    Temple 46 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Crypto Prices
    """

    API_BASE = "https://api.coingecko.com/api/v3"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("COINGECKO_API_KEY")
        if not self.token and "COINGECKO_API_KEY" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing COINGECKO_API_KEY. "
                f"Get your token from the CoinGecko developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "COINGECKO_API_KEY" == "NO_AUTH_REQUIRED":
