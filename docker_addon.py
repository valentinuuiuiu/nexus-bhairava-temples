"""
Docker Addon for NewZyon/PAI-KAN Architecture
Temple 14 - Docker Guardian
Purpose: Containers
...
class DockerAddon:
    """
    Docker integration addon for MCP server
    Temple 14 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Containers
    """

    API_BASE = "https://hub.docker.com/v2"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("DOCKER_TOKEN")
        if not self.token and "DOCKER_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing DOCKER_TOKEN. "
                f"Get your token from the Docker developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "DOCKER_TOKEN" == "NO_AUTH_REQUIRED":
