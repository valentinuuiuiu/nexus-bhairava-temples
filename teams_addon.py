"""
MS Teams Addon for NewZyon/PAI-KAN Architecture
Temple 40 - MS Teams Guardian
...
class TeamsAddon:
    """
    MS Teams integration addon for MCP server
    Temple 40 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Microsoft Collaboration
    """

    API_BASE = "https://graph.microsoft.com/v1.0"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("TEAMS_TOKEN")
        if not self.token and "TEAMS_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing TEAMS_TOKEN. "
                f"Get your token from the MS Teams developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "TEAMS_TOKEN" == "NO_AUTH_REQUIRED":
