"""
Twitter/X Addon for NewZyon/PAI-KAN Architecture
Temple 33 - Twitter/X Guardian
...
class TwitterAddon:
    """
    Twitter/X integration addon for MCP server
    Temple 33 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Social Media
    """

    API_BASE = "https://api.twitter.com/2"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("TWITTER_BEARER_TOKEN")
        if not self.token and "TWITTER_BEARER_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing TWITTER_BEARER_TOKEN. "
                f"Get your token from the Twitter/X developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "TWITTER_BEARER_TOKEN" == "NO_AUTH_REQUIRED":
