"""
WeatherAPI Addon for NewZyon/PAI-KAN Architecture
Temple 63 - WeatherAPI Guardian
...
class WeatherAddon:
    """
    WeatherAPI integration addon for MCP server
    Temple 63 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Weather Data
    """

    API_BASE = "https://api.weatherapi.com/v1"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("WEATHER_API_KEY")
        if not self.token and "WEATHER_API_KEY" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing WEATHER_API_KEY. "
                f"Get your token from the WeatherAPI developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "WEATHER_API_KEY" == "NO_AUTH_REQUIRED":
