"""
NewZyon/KAN Addon for NewZyon/PAI-KAN Architecture
Temple 64 - NewZyon/KAN Guardian
...
class NewzyonAddon:
    """
    NewZyon/KAN integration addon for MCP server
    Temple 64 of the 64 Bhairava Guardian Temples
    Guardian Purpose: PAI-KAN Sacred Core
    """

    API_BASE = "http://localhost:8000/api"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("NEWZYON_TOKEN")
        if not self.token and "NEWZYON_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing NEWZYON_TOKEN. "
                f"Get your token from the NewZyon/KAN developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "NEWZYON_TOKEN" == "NO_AUTH_REQUIRED":
