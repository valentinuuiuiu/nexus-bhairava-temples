"""
Discord Addon for NewZyon/PAI-KAN Architecture
Temple 31 - Discord Guardian
...
class DiscordAddon:
    """
    Discord integration addon for MCP server
    Temple 31 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Gaming & Community
    """
    ...
    def __init__(self, token: Optional[str] = None):
        ...
        if not self.token and "DISCORD_BOT_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing DISCORD_BOT_TOKEN. "
                f"Get your token from the Discord developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "DISCORD_BOT_TOKEN" == "NO_AUTH_REQUIRED":
