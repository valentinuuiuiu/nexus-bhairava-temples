"""
Anthropic Addon for NewZyon/PAI-KAN Architecture
Temple 27 - Anthropic Guardian
...
class AnthropicAddon:
    """
    Anthropic integration addon for MCP server
    Temple 27 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Claude Models
    """

    API_BASE = "https://api.anthropic.com/v1"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("ANTHROPIC_API_KEY")
        if not self.token and "ANTHROPIC_API_KEY" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing ANTHROPIC_API_KEY. "
                f"Get your token from the Anthropic developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "ANTHROPIC_API_KEY" == "NO_AUTH_REQUIRED":
