"""
Splunk Addon for NewZyon/PAI-KAN Architecture
Temple 60 - Splunk Guardian
...
class SplunkAddon:
    """
    Splunk integration addon for MCP server
    Temple 60 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Log Management
    """

    API_BASE = "https://localhost:8089/services"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("SPLUNK_TOKEN")
        if not self.token and "SPLUNK_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing SPLUNK_TOKEN. "
                f"Get your token from the Splunk developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "SPLUNK_TOKEN" == "NO_AUTH_REQUIRED":
