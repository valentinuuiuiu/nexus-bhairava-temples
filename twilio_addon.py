"""
Twilio Addon for NewZyon/PAI-KAN Architecture
Temple 37 - Twilio Guardian
Purpose: SMS & Voice
...
class TwilioAddon:
    """
    Twilio integration addon for MCP server
    Temple 37 of the 64 Bhairava Guardian Temples
    Guardian Purpose: SMS & Voice
    """
    ...
    def __init__(self, token: Optional[str] = None):
        ...
        if not self.token and "TWILIO_AUTH_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing TWILIO_AUTH_TOKEN. "
                f"Get your token from the Twilio developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "TWILIO_AUTH_TOKEN" == "NO_AUTH_REQUIRED":
