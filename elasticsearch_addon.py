"""
Elasticsearch Addon for NewZyon/PAI-KAN Architecture
Temple 24 - Elasticsearch Guardian
...
class ElasticsearchAddon:
    """
    Elasticsearch integration addon for MCP server
    Temple 24 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Search Engine
    """

    API_BASE = "http://localhost:9200"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("ES_URL")
        if not self.token and "ES_URL" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing ES_URL. "
                f"Get your token from the Elasticsearch developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "ES_URL" == "NO_AUTH_REQUIRED":
