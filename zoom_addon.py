"""
Zoom Addon for NewZyon/PAI-KAN Architecture
Temple 39 - Zoom Guardian
...
class ZoomAddon:
    """
    Zoom integration addon for MCP server
    Temple 39 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Video Meetings
    """

    API_BASE = "https://api.zoom.us/v2"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("ZOOM_TOKEN")
        if not self.token and "ZOOM_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing ZOOM_TOKEN. "
                f"Get your token from the Zoom developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "ZOOM_TOKEN" == "NO_AUTH_REQUIRED":
