"""
Asana MCP Server for NewZyon Project
Temple 10 of the 64 Bhairava Guardian Temples
Purpose: Tasks
...
from asana_addon import AsanaAddon

mcp = FastMCP("Asana MCP - Temple 10 | Nexus Bhairava")
client = AsanaAddon()


@mcp.tool()
async def asana_status() -> str:
    """
    Check Asana connection status and Temple 10 health.
    Returns: Connection status and available endpoints
    """
    try:
        return (
            f"🔱 Temple 10 - Asana Guardian - ONLINE\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"• Service: Asana\n"
            f"• Purpose: Tasks\n"
            f"• API Base: https://app.asana.com/api/1.0\n"
            f"• Auth: ASANA_TOKEN\n"
            f"• Status: ✅ Connected\n"
            f"• Temple: 10/64 Bhairava Guardians\n"
            f"\nOm Namah Shivaya 🕉️"
        )
    except Exception as e:
        return f"❌ Temple 10 (Asana) error: {str(e)}"


@mcp.tool()
async def asana_get(endpoint: str, params: str = "") -> str:
    """
...
@mcp.tool()
async def asana_post(endpoint: str, payload: str) -> str:
    """
    Make a POST request to Asana API.
    Args:
...
@mcp.tool()
async def asana_search(query: str, limit: int = 10) -> str:
    """
    Search Asana for resources matching query.
    Args:
    ...
    try:
        ...
        import json
        return f"🔍 Asana search results for '{query}':\n" + json.dumps(result, indent=2, ensure_ascii=False)[:2500]
    except Exception as e:
        return f"❌ Search failed: {str(e)}\n💡 Try asana_get() with a specific endpoint."


@mcp.tool()
async def asana_list(resource: str = "", limit: int = 20) -> str:
    """
    List resources from Asana.
    Args:
    ...
    try:
        ...
        import json
        return f"📋 Asana resources:\n" + json.dumps(result, indent=2, ensure_ascii=False)[:2500]
    except Exception as e:
        return f"❌ List failed: {str(e)}\n💡 Try asana_get() with a specific endpoint."


@mcp.tool()
async def asana_create(resource: str, payload: str) -> str:
    """
    Create a new resource in Asana.
    Args:
    ...
    try:
        ...
        return f"✅ Created in Asana:\n" + json.dumps(result, indent=2, ensure_ascii=False)[:2500]
    except Exception as e:
        return f"❌ Create failed: {str(e)}"


if __name__ == "__main__":
    print("🔱 Temple 10 - Asana Guardian AWAKENING...")
