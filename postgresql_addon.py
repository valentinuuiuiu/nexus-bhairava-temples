"""
PostgreSQL Addon for NewZyon/PAI-KAN Architecture
Temple 21 - PostgreSQL Guardian
...
class PostgresqlAddon:
    """
    PostgreSQL integration addon for MCP server
    Temple 21 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Relational DB
    """

    API_BASE = "postgresql://localhost:5432"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("DATABASE_URL")
        if not self.token and "DATABASE_URL" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing DATABASE_URL. "
                f"Get your token from the PostgreSQL developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "DATABASE_URL" == "NO_AUTH_REQUIRED":
