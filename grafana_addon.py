"""
Grafana Addon for NewZyon/PAI-KAN Architecture
Temple 51 - Grafana Guardian
Purpose: Monitoring
...
class GrafanaAddon:
    """
    Grafana integration addon for MCP server
    Temple 51 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Monitoring
    """
    ...
    def __init__(self, token: Optional[str] = None):
        ...
        if not self.token and "GRAFANA_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing GRAFANA_TOKEN. "
                f"Get your token from the Grafana developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "GRAFANA_TOKEN" == "NO_AUTH_REQUIRED":
