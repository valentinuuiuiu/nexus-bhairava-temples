"""
SendGrid Addon for NewZyon/PAI-KAN Architecture
Temple 36 - SendGrid Guardian
...
class SendgridAddon:
    """
    SendGrid integration addon for MCP server
    Temple 36 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Email Delivery
    """

    API_BASE = "https://api.sendgrid.com/v3"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("SENDGRID_API_KEY")
        if not self.token and "SENDGRID_API_KEY" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing SENDGRID_API_KEY. "
                f"Get your token from the SendGrid developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "SENDGRID_API_KEY" == "NO_AUTH_REQUIRED":
