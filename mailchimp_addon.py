"""
Mailchimp Addon for NewZyon/PAI-KAN Architecture
Temple 38 - Mailchimp Guardian
...
class MailchimpAddon:
    """
    Mailchimp integration addon for MCP server
    Temple 38 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Email Marketing
    """

    API_BASE = "https://us1.api.mailchimp.com/3.0"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("MAILCHIMP_API_KEY")
        if not self.token and "MAILCHIMP_API_KEY" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing MAILCHIMP_API_KEY. "
                f"Get your token from the Mailchimp developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "MAILCHIMP_API_KEY" == "NO_AUTH_REQUIRED":
