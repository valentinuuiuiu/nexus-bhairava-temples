"""
Azure Addon for NewZyon/PAI-KAN Architecture
Temple 13 - Azure Guardian
...
class AzureAddon:
    """
    Azure integration addon for MCP server
    Temple 13 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Microsoft Cloud
    """

    API_BASE = "https://management.azure.com"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("AZURE_TOKEN")
        if not self.token and "AZURE_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing AZURE_TOKEN. "
                f"Get your token from the Azure developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "AZURE_TOKEN" == "NO_AUTH_REQUIRED":
