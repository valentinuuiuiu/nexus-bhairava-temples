"""
PagerDuty Addon for NewZyon/PAI-KAN Architecture
Temple 54 - PagerDuty Guardian
...
class PagerdutyAddon:
    """
    PagerDuty integration addon for MCP server
    Temple 54 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Incident Management
    """

    API_BASE = "https://api.pagerduty.com"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("PAGERDUTY_TOKEN")
        if not self.token and "PAGERDUTY_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing PAGERDUTY_TOKEN. "
                f"Get your token from the PagerDuty developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "PAGERDUTY_TOKEN" == "NO_AUTH_REQUIRED":
