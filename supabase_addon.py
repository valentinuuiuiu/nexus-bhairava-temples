"""
Supabase Addon for NewZyon/PAI-KAN Architecture
Temple 29 - Supabase Guardian
...
class SupabaseAddon:
    """
    Supabase integration addon for MCP server
    Temple 29 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Open Source Firebase
    """

    API_BASE = "https://supabase.com/api"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("SUPABASE_KEY")
        if not self.token and "SUPABASE_KEY" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing SUPABASE_KEY. "
                f"Get your token from the Supabase developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "SUPABASE_KEY" == "NO_AUTH_REQUIRED":
