"""
QuickBooks Addon for NewZyon/PAI-KAN Architecture
Temple 47 - QuickBooks Guardian
Purpose: Accounting
...
class QuickbooksAddon:
    """
    QuickBooks integration addon for MCP server
    Temple 47 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Accounting
    """

    API_BASE = "https://quickbooks.api.intuit.com/v3"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("QB_ACCESS_TOKEN")
        if not self.token and "QB_ACCESS_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing QB_ACCESS_TOKEN. "
                f"Get your token from the QuickBooks developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "QB_ACCESS_TOKEN" == "NO_AUTH_REQUIRED":
