"""
Okta Addon for NewZyon/PAI-KAN Architecture
Temple 57 - Okta Guardian
Purpose: Identity
...
class OktaAddon:
    """
    Okta integration addon for MCP server
    Temple 57 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Identity
    """

    API_BASE = "https://yourdomain.okta.com/api/v1"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("OKTA_TOKEN")
        if not self.token and "OKTA_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing OKTA_TOKEN. "
                f"Get your token from the Okta developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "OKTA_TOKEN" == "NO_AUTH_REQUIRED":
