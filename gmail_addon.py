"""
Gmail Addon for NewZyon/PAI-KAN Architecture
Temple 6 - Gmail Guardian
Purpose: Email
...
class GmailAddon:
    """
    Gmail integration addon for MCP server
    Temple 6 of the 64 Bhairava Guardian Temples
    Guardian Purpose: Email
    """

    API_BASE = "https://gmail.googleapis.com"

    def __init__(self, token: Optional[str] = None):
        self.token = token or os.getenv("GMAIL_TOKEN")
        if not self.token and "GMAIL_TOKEN" != "NO_AUTH_REQUIRED":
            raise ValueError(
                f"Missing GMAIL_TOKEN. "
                f"Get your token from the Gmail developer portal."
            )
        self.headers = self._build_headers()

    def _build_headers(self) -> Dict[str, str]:
        if "GMAIL_TOKEN" == "NO_AUTH_REQUIRED":
